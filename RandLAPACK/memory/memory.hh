...
Status malloc(size_t len, value_t **values)
{
    ...
    throw std::invalid_argument(
        "Compiled without Device::CUDA.");
#endif
    }
    else
    {
        throw std::invalid_argument(
            "Device has to be either RandLAPACK::Device::CPU or RandLAPACK::Device::CUDA");
    }
} // malloc
...
struct StridedStorage
{
    ...
    StridedStorage(dim<2> size, size_t lead_dim, Layout layout, NumericPtrVariant values)
...
struct CsrStorage
{
    ...
    CsrStorage() = default;
    CsrStorage(dim<2> size, size_t nnz, NumericType value_type, IntType index_type)
        : size_{size}, nnz_{nnz} {}
    CsrStorage(dim<2> size, size_t nnz, NumericPtrVariant values, IntPtrVariant row_ptrs, IntPtrVariant col_idxs, IntType)
        : size_{size}, nnz_{nnz}, values_{values}, row_ptrs_{row_ptrs}, col_idxs_{col_idxs} {}
}; // struct CsrStorage
